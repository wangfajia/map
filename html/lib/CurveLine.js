...
(function() {
    ...
    /** 
     * @exports CurveLine as BMapLib.CurveLine
     */
    var CurveLine =
        /**
         * CurveLine类的构造函数
    ...
    getCurveByTwoPoints = function(obj1, obj2) {
        ...
        for (i = 0; i < count + 1; i++) {
            curveCoordinates.push(new BMap.Point(
                (lng1 * B1(inc) + lng3 * B2(inc)) + lng2 * B3(inc), (lat1 * B1(inc) + lat3 * B2(inc) + lat2 * B3(inc))
            ));
            inc = inc + (1 / count);
        }
        ...
        if (this.map) {
            ...
            for (var i = 0; i < this.points.length; i++) {
                var marker = new BMap.Marker(this.points[i], {
                    icon: new BMap.Icon('images/circle.png', new BMap.Size(16, 16)),
                    enableDragging: true,
                    raiseOnDrag: true
                });
                var self = this;
